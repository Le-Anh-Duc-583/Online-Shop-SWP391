  ...
  try {
    ...
    if (!billDetailID) {
      ...
      return;
        ...
        const result = await sql.query`
            SELECT p.ProductName , p.ProductPrice, p.ProductPic
            FROM BillDetails bd
...
const getCart = async (req, res) => {
  try {
    ...
    const result =
      await sql.query`SELECT * FROM Bills where BillStatus like N'Chưa thanh toán' and UserID = ${id}`;
    res.json(result.recordset);
  } catch (err) {
    console.error(err);
    res.status(500).send("Server Error");
...
const getBillDetailByBillID = async (req, res) => {
  try {
    ...
    const result =
      await sql.query`SELECT * FROM BillDetails where BillID = ${billID}`;
    res.json(result.recordset);
  } catch (err) {
    console.error(err);
    res.status(500).send("Server Error");
...
const addNewBill = async (req, res) => {
  try {
    const id = req.params.id;
    const result =
      await sql.query`INSERT INTO Bills(UserID, BillStatus) VALUES (${id}, N'Chưa thanh toán')`;
    res.json(result.recordset);
  } catch (err) {
    console.error(err);
    res.status(500).send("Server Error");
...
const deleteBill = async (req, res) => {
  try {
    const id = req.params.id;
    const result =
      await sql.query`DELETE FROM BillDetails WHERE BillDetailID = ${id}`;
...
const addToCart = async (req, res) => {
  try {
    const { BillID, ProductID, BillDetailDate, BillDetailQuantity, ShipperID } =
      req.body;
    await sql.query`
